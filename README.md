...
フロア図は実ピクセルサイズのまま表示するため、大きすぎる画像をそのまま許すと延々スクロールすることになる。アップロード時（[`/api/floors/[floorId]/image`](./src/app/api/floors/[floorId]/image/route.ts)）に、長辺が1920pxを超える画像はsharpで自動的に縮小する（縦横比は維持、小さい画像を拡大はしない）。既存の会議室がある状態で差し替えて画像サイズが変わる場合、そのままだと会議室の座標が新しい画像に対してずれてしまうため、差し替え前後のサイズ比で全会議室の位置・サイズを比例配分して補正する（フロア図の更新と同じDBトランザクション内で実施）。
