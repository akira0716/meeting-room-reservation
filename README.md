...
- サインインは初回のみマジックリンク（メール所有確認）→パスワード設定、2回目以降はメール+パスワードの2段階
  - 招待URL方式：管理者が管理ページから招待→招待された人がリンクを踏んでメール認証→パスワード設定→組織に参加
  - 初期管理者はconfigファイルでシード（「最初の管理者を誰が招待するか」という鶏卵問題の解消）→同じくマジックリンクで初回サインイン→パスワード設定
