...
- [x] **レイアウト変更**：フロアマップを画面横いっぱいに表示するよう変更し、予約状況（`RoomDetailPanel`）専用の右カラムを廃止。クリックした会議室の右隣（収まらなければ左隣）に、フロアマップ上へ重ねるポップオーバーとして表示するようにした（`getScreenCTM()`で会議室のSVG座標を画面座標→コンテナのコンテンツ座標に変換。ネイティブ表示・縮小表示どちらでもスクロールしても正しい位置に追従する）。`RoomDetailPanel`に`onClose`を追加し閉じるボタンを表示。編集モードの「＋会議室を追加」等のパネルはマップ下に全幅で表示するよう変更
