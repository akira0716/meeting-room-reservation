...
- [x] 会議室をクリックして予約状況ポップオーバーが表示されると、ヘルプテキスト（「会議室をクリックすると...」）が消えてページの高さがわずかに変わり、ブラウザの縦スクロールバーが出たり消えたりしてコンテンツの見た目の幅がガタつく問題を修正。`html`に`overflow-y: scroll`を指定し、スクロールバー分の余白を常に確保するようにした（`scrollbar-gutter: stable`はこの検証環境では効果が確認できなかったため、より確実な`overflow-y: scroll`を採用）
